...
mod amiigen {
    ...
    pub fn gen_amiibo(amiibo_id: [u8; 8], tag_uid: &[u8]) -> Result<[u8; 540], AmiigenError> {
        if tag_uid.len() < 7 {
          return Err(AmiigenError::InvalidData("Please use a valid 7 or 9 byte uid".to_string())); 
        } 
        let (small_uid, bcc1, uid) = match tag_uid.len() {
            7 => { 
              ...
              Ok((tag_uid.try_into().unwrap(), bcc1, [tag_uid[0], tag_uid[1], tag_uid[2], bcc0, tag_uid[3], tag_uid[4], tag_uid[5], tag_uid[6]]))
            }
            9 => {
              let small_uid = [tag_uid[0], tag_uid[1], tag_uid[2], tag_uid[4], tag_uid[5], tag_uid[6], tag_uid[7]]; 
              Ok((small_uid, tag_uid[8], tag_uid[0..8].try_into().unwrap())) // meant to be inclusive
            }
            _ => Err(AmiigenError::InvalidData("Please use a valid 7 or 9 byte uid".to_string()))
        }?;
